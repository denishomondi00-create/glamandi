...
export class PropertiesService {
  constructor(@InjectModel(Property.name) private readonly model: Model<PropertyDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!record) throw new EntityNotFoundException('Property', id);
    return record;
  }
  ...
  async update(id: string, dto: UpdatePropertyDto) {
    ...
    if (!updated) throw new EntityNotFoundException('Property', id);
    return updated;
  }

  async remove(id: string) {
    const removed = await this.model.findByIdAndUpdate(id, { $set: { deleted_at: new Date() } }, { new: true }).lean();
    if (!removed) throw new EntityNotFoundException('Property', id);
    return removed;
  }
}
