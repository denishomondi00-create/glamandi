...
import { WebsiteListing, WebsiteListingDocument } from '../../database/schemas/website-listing.schema';
import { CreateWebsiteDto } from './dto/create-website.dto';
import { UpdateWebsiteDto } from './dto/update-website.dto';

@Injectable()
export class WebsiteService {
  constructor(@InjectModel(WebsiteListing.name) private readonly model: Model<WebsiteListingDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!record) throw new EntityNotFoundException('Website', id);
    return record;
  }
  ...
  async update(id: string, dto: UpdateWebsiteDto) {
    ...
    if (!updated) throw new EntityNotFoundException('Website', id);
    return updated;
  }

  async remove(id: string) {
    const removed = await this.model.findByIdAndUpdate(id, { $set: { deleted_at: new Date() } }, { new: true }).lean();
    if (!removed) throw new EntityNotFoundException('Website', id);
    return removed;
  }
}
