...
export class DepositsService {
  constructor(@InjectModel(DepositLedger.name) private readonly model: Model<DepositLedgerDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!record) throw new EntityNotFoundException('Deposit', id);
    return record;
  }
  ...
  async update(id: string, dto: UpdateDepositDto) {
    ...
    if (!updated) throw new EntityNotFoundException('Deposit', id);
    return updated;
  }

  async remove(id: string) {
    const removed = await this.model.findByIdAndUpdate(id, { $set: { deleted_at: new Date() } }, { new: true }).lean();
    if (!removed) throw new EntityNotFoundException('Deposit', id);
    return removed;
  }
}
