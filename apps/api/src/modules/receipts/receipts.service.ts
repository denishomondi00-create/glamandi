...
import { Receipt, ReceiptDocument } from '../../database/schemas/receipt.schema';
import { CreateReceiptDto } from './dto/create-receipt.dto';
import { UpdateReceiptDto } from './dto/update-receipt.dto';

@Injectable()
export class ReceiptsService {
  constructor(@InjectModel(Receipt.name) private readonly model: Model<ReceiptDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!record) throw new EntityNotFoundException('Receipt', id);
    return record;
  }
  ...
  async update(id: string, dto: UpdateReceiptDto) {
    ...
    if (!updated) throw new EntityNotFoundException('Receipt', id);
    return updated;
  }

  async remove(id: string) {
    const removed = await this.model.findByIdAndUpdate(id, { $set: { deleted_at: new Date() } }, { new: true }).lean();
    if (!removed) throw new EntityNotFoundException('Receipt', id);
    return removed;
  }
}
