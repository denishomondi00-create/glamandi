...
export class TenanciesService {
  constructor(@InjectModel(Tenancy.name) private readonly model: Model<TenancyDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!record) throw new EntityNotFoundException('Tenancy', id);
    return record;
  }
  ...
  async update(id: string, dto: UpdateTenancyDto) {
    ...
    if (!updated) throw new EntityNotFoundException('Tenancy', id);
    return updated;
  }

  async remove(id: string) {
    const removed = await this.model.findByIdAndUpdate(id, { $set: { deleted_at: new Date() } }, { new: true }).lean();
    if (!removed) throw new EntityNotFoundException('Tenancy', id);
    return removed;
  }
}
