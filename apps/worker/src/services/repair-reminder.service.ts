...
@Injectable()
export class RepairReminderService {
  constructor(
  ...
  async handle(payload: Record<string, unknown> = {}, job?: Job) {
    this.logger.info('Flag overdue repairs and notify staff.', { jobId: job?.id, jobName: job?.name, payload });
    const endpoint = '/repairs/reminders/run';
